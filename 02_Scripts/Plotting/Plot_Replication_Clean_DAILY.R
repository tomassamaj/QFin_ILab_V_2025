...
# Load industry names
ind_ports <- read.csv("01_Data/Raw/17_Industry_Portfolios.csv")
...
if (!is.null(industry_momentum) && !is.null(factor_momentum)) {
  ...
  print(
    ggplot(
      ...
      labs(
        title = "Cumulative Performance (Strict Monthly Rebalancing)",
...
scale_volatility <- function(
  ...
  df %>%
    mutate(scaled_momentum_return = momentum_return * leverage_factor) %>%
...
if (!is.null(industry_momentum_scaled) && !is.null(factor_momentum_scaled)) {
  ...
  print(
    ggplot(
      ...
      scale_y_log10() +
      labs(
        title = "Scaled Performance (Strict Monthly Rebalancing)",
        y = "Cumulative Performance ($)"
      )
  )
}
...
reg_data <- final_merged_renamed %>%
  ...
  inner_join(ff3_factors_daily, by = "date") %>%
...
ls_plot <- ggplot(
  ...
  annotate(
    ...
    xmin = ymd("2000-03-01"),
    xmax = ymd("2002-10-01"),
    ymin = min(plot_data_final$Cumulative_Return),
    ymax = max(plot_data_final$Cumulative_Return),
    alpha = 0.2,
    fill = "gray50"
  ) +
  geom_line(linewidth = 1) +
  ...
  labs(
    title = "Factor Momentum (21d Lookback, Monthly Rebal)",
