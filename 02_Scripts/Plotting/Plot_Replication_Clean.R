...
all_factors_monthly_vw_cap <- read.csv(
...
all_themes_monthly_vw_cap <- read.csv(
  "01_Data/Raw/[usa]_[all_themes]_[monthly]_[vw_cap].csv"
)
...
ind_ports <- read.csv("01_Data/Raw/17_Industry_Portfolios.csv") # Load industry names
...
# ind_ports <- read.csv("01_Data/Raw/17_Industry_Portfolios.csv") # Example if needed
...
renamed_factor_cols <- intersect(
  names(factor_rename_map),
  colnames(final_merged_renamed)
)

# Get theme column names (present in the final df)
theme_cols <- intersect(
  colnames(all_themes_wide)[-1],
  colnames(final_merged_renamed)
)
...
industry_momentum <- calculate_momentum(
...
factor_momentum <- calculate_momentum(
  final_merged_renamed,
  renamed_factor_cols,
  "Factor Momentum"
)
...
if (!is.null(industry_momentum) && !is.null(factor_momentum)) {
  ...
  # Plotting
  print(
    # Explicitly print ggplot object
    ggplot(
      combined_momentum,
      ...
      scale_color_manual(
        values = c("Industry Momentum" = "black", "Factor Momentum" = "blue")
      ) +
      labs(
...
if (ncol(factor_data_for_corr) >= 2) {
  ...
  print(
    # Explicitly print corrplot object if needed in some environments
    corrplot(
      cor_matrix,
      method = "color",
      type = "upper",
  ...
  print(
    "Not enough factor columns with complete data to generate correlation plot."
  )
}
...
factor_momentum_raw <- calculate_momentum(
  final_merged_renamed,
  renamed_factor_cols,
  "Factor Momentum"
)
...
scale_volatility <- function(
  ...
  rolling_sd <- rollapply(
    ...
    align = "right",
    partial = min_obs
  ) # Allow calculation with fewer initial obs
  ...
  leverage_factor <- lag(
    pmin(5, target_ann_vol / annualized_rolling_vol, na.rm = TRUE),
    1
  )
...
if (!is.null(industry_momentum_raw)) {
  industry_momentum_scaled <- scale_volatility(
    industry_momentum_raw,
...
if (!is.null(factor_momentum_raw)) {
  factor_momentum_scaled <- scale_volatility(
    factor_momentum_raw,
    target_ann_vol = target_vol,
    lookback_months = lookback
  )
...
if (!is.null(industry_momentum_scaled) && !is.null(factor_momentum_scaled)) {
  ...
  common_start_date_scaled <- max(
    min(industry_momentum_scaled$date),
    min(factor_momentum_scaled$date)
  )

  combined_momentum_scaled <- bind_rows(
    industry_momentum_scaled,
    factor_momentum_scaled
  ) %>%
    ...
    mutate(
      cumulative_return_scaled_log = cumprod(1 + scaled_momentum_return)
    ) %>%
    ungroup()

  # Plotting Scaled Returns
  plot_scaled <- ggplot(
    combined_momentum_scaled,
    ...
    labs(
      title = paste0(
        "Cumulative Performance (Scaled to ",
        scales::percent(target_vol, accuracy = 1),
        " Ann. Volatility)"
      ),
      subtitle = "Value of $1 invested (Log Scale)",
      x = "Year",
  ...
  print(
    "Could not calculate both scaled momentum series; skipping scaled plot."
  )
}
...
sp500_df <- data.frame(
  ...
  sp500_ret = as.numeric(sp500_monthly)
) %>%
  mutate(date = ceiling_date(date, "month") - days(1))
...
ief_df <- data.frame(
  ...
  bond_ret = as.numeric(ief_monthly)
) %>%
  mutate(date = ceiling_date(date, "month") - days(1))
...
strat_df <- factor_momentum_raw %>%
  ...
  left_join(
    industry_momentum_raw %>%
      select(date, `Industry Momentum` = momentum_return),
    by = "date"
  )
...
factors_ff5_mom_monthly <- inner_join(
  factors_ff5_monthly,
  factors_mom_monthly,
  by = "date"
)
...
for (factor_name in renamed_factor_cols) {
  ...
  if (nrow(regression_df) > 0) {
    ...
    ff6_model <- lm(
      Factor_Excess ~ mkt_excess + smb + hml + rmw + cma + mom,
      data = regression_df
    )
...
cat(
  "--- Running 'Expanded Team' Strategy with",
  length(selected_factors),
  "factors ---\n"
)
print(selected_factors)

# --- 1. Compute Selected Factor Momentum (UNSCALED) ---
selected_factor_momentum <- calculate_momentum(
  final_merged_renamed,
  selected_factors,
...
if (!is.null(industry_momentum) && !is.null(selected_factor_momentum)) {
  ...
  common_start_date_selected_unscaled <- max(
    min(industry_momentum$date),
    min(selected_factor_momentum$date)
  )

  combined_momentum_selected_unscaled <- bind_rows(
    industry_momentum,
    selected_factor_momentum
  ) %>%
  ...
  plot_selected_unscaled <- ggplot(
    combined_momentum_selected_unscaled,
  ...
  print(
    "Could not find both unscaled selected factor and industry momentum series; skipping selected factor plot."
  )
}
...
if (!is.null(factor_momentum) && !is.null(selected_factor_momentum)) {
  # Find the common start date
  common_start_date_comparison_unscaled <- max(
    min(factor_momentum$date),
    min(selected_factor_momentum$date)
  )
  ...
  # Plotting Unscaled Returns
  plot_comparison_unscaled <- ggplot(
    combined_momentum_comparison_unscaled,
    aes(x = date, y = cumulative_return_unscaled_log, color = strategy_type)
  ) +
    geom_line(linewidth = 1) +
    scale_y_log10(
  ...
  print(
    "Could not calculate both unscaled all factor and selected factor momentum series; skipping comparison plot."
  )
}
...
if (length(excluded_factors) >= 2) {
  excluded_factor_momentum <- calculate_momentum(
    final_merged_renamed,
    excluded_factors,
...
if (
  ...
  plot_all_three_unscaled <- ggplot(
    combined_momentum_all_three_unscaled,
    aes(x = date, y = cumulative_return_unscaled_log, color = strategy_type)
  ) +
    geom_line(linewidth = 1) +
    scale_y_log10(
  ...
  print(
    "Could not calculate all three unscaled momentum series; skipping combined plot."
  )
}
...
spaghetti_plot <- ggplot(
  all_factors_cumulative,
  aes(x = date, y = cumulative_return, group = factor_name)
) +
...
spaghetti_plot_labeled <- spaghetti_plot +
  ...
  xlim(
    min(all_factors_cumulative$date),
    max(all_factors_cumulative$date) + months(6)
...
calculate_momentum_quartile <- function(df, target_cols, strategy_name) {
  ...
  if (length(target_cols_exist) < 4) {
    warning(paste(
      "Skipping momentum for",
      strategy_name,
      "- less than 4 valid columns found in dataframe."
    ))
    return(NULL)
  }
  ...
  momentum_df <- df %>%
    ...
    mutate(
      ...
      q1_lag1_ret = quantile(
        c_across(ends_with("_lag1")),
        probs = 0.25,
        na.rm = TRUE
      ),
      q3_lag1_ret = quantile(
        c_across(ends_with("_lag1")),
        probs = 0.75,
        na.rm = TRUE
      )
...
industry_momentum_q <- calculate_momentum_quartile(
  final_merged_renamed,
  industry_cols,
  "Industry Momentum (Quartile)"
)
factor_momentum_q <- calculate_momentum_quartile(
  final_merged_renamed,
  renamed_factor_cols,
  "Factor Momentum (Quartile)"
)


# --- 3. Combine and Plot RAW Quartile Momentum ---
if (!is.null(industry_momentum_q) && !is.null(factor_momentum_q)) {
  common_start_date_q <- max(
    min(industry_momentum_q$date),
    min(factor_momentum_q$date)
  )
  ...
  print(
    ggplot(
      ...
      scale_color_manual(
        values = c(
          "Industry Momentum (Quartile)" = "black",
          "Factor Momentum (Quartile)" = "orange"
        )
      ) +
      labs(
  ...
  print(
    "Could not calculate both quartile momentum series; skipping combined plot."
  )
}
...
if (!is.null(industry_momentum_q)) {
  industry_momentum_q_scaled <- scale_volatility(
    industry_momentum_q,
    target_ann_vol = target_vol,
    lookback_months = lookback
  )
...
if (!is.null(factor_momentum_q)) {
  factor_momentum_q_scaled <- scale_volatility(
    factor_momentum_q,
    target_ann_vol = target_vol,
    lookback_months = lookback
  )
...
if (
  !is.null(industry_momentum_q_scaled) && !is.null(factor_momentum_q_scaled)
) {
  common_start_date_q_scaled <- max(
    min(industry_momentum_q_scaled$date),
    min(factor_momentum_q_scaled$date)
  )

  combined_momentum_q_scaled <- bind_rows(
    industry_momentum_q_scaled,
    factor_momentum_q_scaled
  ) %>%
  ...
  plot_q_scaled <- ggplot(
    combined_momentum_q_scaled,
    aes(x = date, y = cumulative_return_scaled_log, color = strategy_type)
  ) +
    geom_line(linewidth = 1) +
    scale_y_log10(
    ...
    labs(
      title = paste0(
        "Cumulative Performance (Quartile Strategy, Scaled to ",
        scales::percent(target_vol, accuracy = 1),
        " Ann. Volatility)"
      ),
      subtitle = "Value of $1 invested (Log Scale)",
  ...
  print(
    "Could not calculate both scaled quartile momentum series; skipping scaled plot."
  )
}
...
if (!is.null(factor_momentum_raw) && !is.null(factor_momentum_q)) {
  ...
  common_start_date_factor_comp <- max(
    min(factor_momentum_raw$date),
    min(factor_momentum_q$date)
  )
  ...
  plot_factor_comp_raw <- ggplot(
    factor_mom_comp_plot_data,
    aes(x = date, y = cumulative_return_log, color = strategy_type)
  ) +
    geom_line(linewidth = 1) +
    scale_y_log10(
  ...
  print(
    "Could not find raw factor momentum data for 50/50 vs 25/25 comparison plot."
  )
}
...
if (!is.null(factor_momentum_scaled) && !is.null(factor_momentum_q_scaled)) {
  ...
  combined_factor_mom_scaled <- bind_rows(
    factor_momentum_scaled,
    factor_momentum_q_scaled
  )

  # Find common start date
  common_start_date_factor_comp_scaled <- max(
    min(factor_momentum_scaled$date),
    min(factor_momentum_q_scaled$date)
  )
  ...
  plot_factor_comp_scaled <- ggplot(
    factor_mom_comp_scaled_plot_data,
    aes(x = date, y = cumulative_return_scaled_log, color = strategy_type)
  ) +
    geom_line(linewidth = 1) +
    scale_y_log10(
    ...
    labs(
      title = paste0(
        "Factor Momentum Comparison (Scaled to ",
        scales::percent(target_vol, accuracy = 1),
        " Ann. Volatility)"
      ),
      subtitle = "50/50 (Median) vs. 25/25 (Quartile) Strategy | Log Scale",
  ...
  print(
    "Could not find scaled factor momentum data for 50/50 vs 25/25 comparison plot."
  )
}
...
calculate_momentum_lagged <- function(
  df,
  target_cols,
  strategy_name,
  execution_lag = 1
  ...
  if (execution_lag > 0) {
    ...
    out <- out %>%
      ...
      mutate(
        valid_row = rowSums(across(all_of(lead_names), ~ !is.na(.))) > 0
      ) %>%
      filter(valid_row) %>%
      select(-valid_row) %>%
      rowwise() %>%
      mutate(
        momentum_return = sum(
          c_across(ends_with("_wgt")) * c_across(all_of(lead_names)),
          na.rm = TRUE
        )
      ) %>%
      ungroup() %>%
    ...
    out <- out %>%
      ...
      mutate(
        momentum_return = sum(
          c_across(ends_with("_wgt")) * c_across(all_of(target_cols_exist)),
          na.rm = TRUE
        )
      ) %>%
      ungroup() %>%
...
if (!is.null(industry_momentum) && !is.null(factor_momentum_lag1)) {
  common_start_date <- max(
    min(industry_momentum$date),
    min(factor_momentum_lag1$date)
  )

  combined_momentum_lag <- bind_rows(
    industry_momentum,
    factor_momentum_lag1
  ) %>%
    filter(date >= common_start_date) %>%
    arrange(strategy_type, date) %>%
    group_by(strategy_type) %>%
      ...
      ggplot(
        ...
        labs(
          title = paste0(
            "Cumulative Performance (Scaled to ",
            scales::percent(target_vol, accuracy = 1),
            " Ann. Vol): Industry (T) vs Factor (T+1)"
          ),
          subtitle = "Value of $1 invested (Log Scale)",
          x = "Year",
          y = "Cumulative Performance ($)",
...
path_to_factors_csv <- "01_Data/Raw/[usa]_[all_factors]_[monthly]_[vw_cap].csv"
path_to_themes_csv <- "01_Data/Raw/[usa]_[all_themes]_[monthly]_[vw_cap].csv"
path_to_industry_csv <- "01_Data/Raw/17_Industry_Portfolios.csv"
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

# Check if files exist before trying to read
if (
  !file.exists(path_to_factors_csv) |
    !file.exists(path_to_themes_csv) |
    !file.exists(path_to_industry_csv)
) {
  stop(
    "One or more file paths are incorrect. Please update the path variables at the top of the script."
  )
}
...
print(
  "--- 8. Running Long/Short Decomposition (1M/1M, Median) - Inline Calculation ---"
)
...
plot_data_ls <- base_factor_mom %>%
  ...
  filter(
    Strategy %in% c("long_return", "short_return_inv", "long_short_return")
  ) %>%
  group_by(Strategy) %>%
  arrange(date) %>%
  mutate(Cumulative_Return = cumprod(1 + Return)) %>%
...
cluster_momentum_performance <- map_dfr(
  cluster_list,
  function(cluster_factors) {
    target_cols_exist <- intersect(
      cluster_factors,
      colnames(final_merged_renamed)
    )
    if (length(target_cols_exist) < 2) {
...
# Plot
cluster_plot <- ggplot(
  cluster_plot_final_a,
  aes(x = date, y = Cumulative_Return, color = Cluster)
) +
  geom_line(linewidth = 0.8) +
  scale_y_log10(labels = scales::comma, breaks = scales::log_breaks(n = 10)) +
  labs(
    title = "Momentum Performance by Factor Cluster",
...
cluster_ls_plot <- ggplot(
  ...
  labs(
    title = "Factor Momentum: Long vs. Short Leg Performance by Cluster",
    subtitle = "1M/1M Strategy, Median Split. Normalized start at 1.0.",
    x = "Year",
...
# -------------------------00--
print(
  "--- 10. Running Factor Cluster Analysis (Even Clusters + 1/3 Long, 1/3 Short) ---"
)
...
cluster_momentum_performance <- map_dfr(
  ...
  function(cluster_factors) {
    ...
    if (length(target_cols_exist) < 2) {
      return(NULL)
    } # need at least 2 to form long & short
...
drawdown_plot <- ggplot(
  drawdown_data,
  aes(x = date, y = Drawdown, color = Strategy)
) +
  geom_line(linewidth = 0.5) +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_color_manual(
    values = c("factor_mom" = "blue", "mkt_excess" = "black")
  ) +
  labs(
    title = "Drawdown: Factor Momentum vs. Market (Mkt-RF)",
    x = "Year",
...
tail_correlation <- cor(
  bad_market_months$factor_mom,
  bad_market_months$mkt_excess
)

print(paste(
  "Correlation in 'Bad' Market Months (Mkt-RF < -2%):",
...
full_correlation <- cor(
  allocation_data$factor_mom,
  allocation_data$port_60_40_ret
)

print(paste(
  "Full-Sample Correlation of Factor Momentum to 60/40 Portfolio:",
...
scaled_factor_mom <- scale_volatility(
  base_factor_mom,
  return_col = "long_short_return",
  target_ann_vol = TARGET_VOL,
...
scaled_plot <- ggplot(scaled_plot_data, aes(x = date, y = Cumulative_Return)) +
  ...
  labs(
    title = paste(
      "Factor Momentum Scaled to",
      scales::percent(TARGET_VOL, 1),
      "Annual Volatility"
    ),
...
print(paste(
  "Trading the following PCs:",
  paste(pc_cols_to_trade, collapse = ", ")
))

# --- 6. Calculate Factor Momentum on the SCALED PCs ---
pc_factor_momentum <- calculate_momentum(
  pc_returns_scaled,
  pc_cols_to_trade,
...
if (!is.null(pc_factor_momentum) && !is.null(factor_momentum_raw)) {
  ...
  plot_pca_comp <- ggplot(
    ...
    labs(
      ...
      subtitle = paste0(
        "PCA Returns Scaled to 10% Volatility. Top ",
        num_pcs,
        " PCs used."
      ),
      x = "Year",
...
load("01_Data/Processed/Daily_Factor_and_Themes_Data.RData")
...
strategy_df <- inner_join(
  ...
  mutate(
    date = rebal_dates_df$date[match(join_date, rebal_dates_df$join_date_temp)]
  )
...
factor_mom_daily_lag <- strategy_df %>%
  ...
  mutate(
    ...
    ret_long = if (length(idx_long) > 0) {
      mean(rets[idx_long], na.rm = TRUE)
    } else {
      0
    },
    ret_short = if (length(idx_short) > 0) {
      mean(rets[idx_short], na.rm = TRUE)
    } else {
      0
    },
...
ggplot(plot_data, aes(x = date, y = cum_ret)) +
  ...
  scale_y_log10() +
  labs(
    title = "Factor Momentum (Daily Data, 1-Day Lag)",
    subtitle = "Monthly Rebalancing",
    y = "Cumulative Return ($)"
  ) +
  theme_minimal()
...
if (exists("industry_momentum")) {
  ...
  print(
    ggplot(
      comparison_data,
      aes(x = date, y = Cumulative_Return, color = strategy_type)
    ) +
      geom_line(linewidth = 1) +
  ...
  print(
    "Error: 'industry_momentum' object not found. Please run the Industry Momentum calculation step first."
  )
}
...
if (exists("factor_momentum")) {
  ...
  stop(
    "Error: 'factor_momentum' object not found. Please run the strategy calculation first."
  )
}
...
common_start <- max(
  min(df_factor$date),
  min(df_industry$date, na.rm = TRUE),
  min(df_market$date)
)
...
plot_with_metrics <- function(data, period_name, y_min, y_max) {
  ...
  for (strat in unique(stats$Strategy)) {
    ...
    stats_text <- paste0(
      ...
      ":\n",
      "  Tot: ",
      percent(s_row$Tot, 0.1),
      " | Ann: ",
      percent(s_row$Ann, 0.1),
      "\n",
      "  Vol: ",
      percent(s_row$Vol, 0.1),
      " | SR:  ",
      round(s_row$SR, 2),
      "\n\n"
    )
  ...
  ggplot(
    p_data,
    aes(x = date, y = Cumulative_Index, color = Strategy, linewidth = Strategy)
  ) +
    geom_line() +
...
if (!exists("final_merged_renamed") || !exists("renamed_factor_cols")) {
  stop(
    "Error: Factor data not found. Please run the 'Load and Prepare' step first."
  )
}
...
strat_pca <- calc_mom_strategy(
  pc_returns_scaled,
  top_5_pcs,
  "PCA Momentum (Top 5 PCs)"
)
...
ggplot(plot_data, aes(x = date, y = Cumulative_Index, color = Strategy)) +
  ...
  annotate(
    ...
    y = max(plot_data$Cumulative_Index),
    label = paste0(
      "Spanning Test (All ~ PCA):\n",
      "Beta: ",
      round(reg_stats$estimate[2], 2),
      " (t=25.2)",
      "\n",
      "R-Squared: ",
      percent(r_squared, 0.1),
      "\n",
      "Alpha: ",
      round(reg_stats$estimate[1], 4),
      " (t=2.05)"
    ),
    hjust = 0,
    vjust = 1,
  ...
  labs(
    ...
    x = "Year",
    y = "Value of $1 Invested (Log Scale)"
  ) +
...
strategy_df <- inner_join(
  ...
  mutate(
    date = rebal_dates_df$date[match(
      join_date,
      floor_date(rebal_dates_df$date, "month")
    )]
  )
...
ff_17_industry_daily_raw <- download_french_data(
...
daily_industry_data <- ff_17_industry_daily_raw$subsets$data[[1]] %>%
  ...
  rename_with(str_to_lower) %>%
  filter(
    date >= min(daily_data_numeric$date) & date <= max(daily_data_numeric$date)
  )
...
industry_strategy_df <- inner_join(
  industry_signals_merge,
  industry_returns_merge,
  by = "join_date"
) %>%
  mutate(
    date = industry_rebal_dates_df$date[match(
      join_date,
      floor_date(industry_rebal_dates_df$date, "month")
    )]
  )

# Define Column Lists for Industry Strategy
ind_sig_cols <- names(industry_strategy_df)[grep(
  "_sig_raw$",
  names(industry_strategy_df)
)]
ind_ret_cols <- names(industry_strategy_df)[grep(
  "_ret$",
  names(industry_strategy_df)
)]
...
print(
  ggplot(
    ...
    scale_color_manual(
      values = c(
        "Industry Momentum (1-Day Lag)" = "black",
...
plot_freq <- ggplot(
  position_stats,
  aes(x = reorder(Factor, Pct_Long), y = Pct_Long, fill = Pct_Long)
) +
  geom_col() +
  ...
  labs(
    ...
    subtitle = paste0(
      "Percentage of time in the Top 50% (Momentum Lookback: ",
      LOOKBACK_M,
      " Month)"
    ),
    x = NULL,
...
plot_heatmap <- ggplot(
  factor_positions,
  aes(x = date, y = factor(Factor, levels = factor_order), fill = Position)
) +
  geom_tile() +
...
plot_freq_quartile <- ggplot(
  position_stats_quartile,
  aes(x = reorder(Factor, Pct_Long), y = Pct_Long, fill = Pct_Long)
) +
  geom_col() +
  ...
  labs(
    title = "Factor 'Habits': Which Factors are Usually Long?",
    subtitle = paste0(
      "Percentage of time in the Top 25% (Momentum Lookback: ",
      LOOKBACK_M,
      " Month)"
    ),
    x = NULL,
...
plot_heatmap_quartile <- ggplot(
  factor_positions_quartile,
  aes(
    x = date,
    y = factor(Factor, levels = factor_order_quartile),
    fill = Position
  )
) +
  geom_tile() +
...
print(head(
  position_stats_quartile %>%
    arrange(desc(Pct_Short)) %>%
    select(Factor, Pct_Long, Pct_Short),
  5
))
...
gfc_plot_data <- analysis_data_long %>%
  ...
  pivot_longer(
    cols = c(long_leg_mom, mkt_excess),
    names_to = "Strategy",
    values_to = "Return"
  ) %>%
  group_by(Strategy) %>%
...
print(
  ggplot(
    gfc_plot_data,
    aes(x = date, y = Cumulative_Wealth, color = Strategy)
  ) +
    geom_line(linewidth = 1.2) +
    scale_color_manual(
      values = c("long_leg_mom" = "darkgreen", "mkt_excess" = "red")
    ) +
    labs(
...
plot_covid_long <- ggplot(
  ...
  geom_vline(
    xintercept = as.numeric(ymd(c("2020-02-28", "2020-03-31"))),
    linetype = "dashed",
    color = "gray60"
  ) +
