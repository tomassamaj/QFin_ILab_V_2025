...
ggplot(perf_ts, aes(x = eom)) +
  ...
  scale_fill_manual(
    ...
    values = c(
      "1. Transaction Costs" = "#FDBF6F", # Light Orange
...
# =============================================================================
#  DRIFT-BAND REBALANCING ANALYSIS (ADD-ON)
# =============================================================================
...
simulate_drift_band <- function(dt_all, threshold_bps) {
  ...
  setkey(dt_all, eom)
  months <- sort(unique(dt_all$eom)) # <-- FIXED: use sort() not setorder()
  ...
  for (i in seq_along(months)) {
    ...
    # Transaction costs (round‑trip) on traded stocks
    month_data[,
    ...
    # Shorting costs (holding cost for short positions)
    month_data[,
...
# --- Run simulations for all thresholds (sequential) ---
cat("\n=============================================================\n")
...
# --- EXTRA PLOT: Cumulative wealth for custom threshold selection ---

# Define which thresholds to display (change as needed)
selected_thresholds <- c(0, 15, 30, 50)

# 1. CRITICAL: Ensure data is sorted by date before calculating cumulative product
setorder(drift_all, threshold, eom)

# 2. Filter first, then compute cumulative wealth
# Note: 'threshold' is removed from the .() because it is already handled by 'by = threshold'
...
# Convert to data.frame for ggplot (ggplot handles data.table, but this is safe)
...
p_extra <- ggplot(
  ...
  theme(
    ...
    plot.title = element_text(face = "bold", color = "#1a2e5a") # Using your WU Navy!
  )
...
# --- ASSUMPTION ---
# Ensure your perf_ts data.table has these two columns:
# 1. ret_net_5bps (The monthly return of the 5bps drift band strategy)
# 2. wealth_net_5bps (The cumulative wealth index: cumprod(1 + ret_net_5bps))
...
ggplot(perf_ts, aes(x = eom)) +
  ...
  scale_color_manual(
    ...
    values = c(
      "Gross (Lagged)" = "#1F78B4", # Blue
      "Net (Final)" = "black",
      "Net (5bps Drift)" = "#c8a951" # Gold to highlight the improvement
    )
  ) +
...
# =============================================================================
# 3. CALCULATE BASELINE FRICTION RATES
# =============================================================================
...
# =============================================================================
# 4. CALCULATE BASELINE COSTS (0 BPS DRIFT)
# =============================================================================
...
perf_ts[, `:=`(
  ret_net_1_tc = ret_gross_lagged - drag_tc,
  ret_net_2_sc = ret_gross_lagged - drag_tc - drag_sc,
  ret_net_3_tax = ret_gross_lagged - drag_tc - drag_sc - drag_tax # Final Baseline Net
)]

# =============================================================================
# 5. DRIFT-BAND REBALANCING SIMULATION
# =============================================================================
...
# =============================================================================
# 6. INTEGRATE RESULTS & CALCULATE WEALTH INDICES
# =============================================================================
cat("[5/5] Integrating results and building wealth indices...\n")

# FIXED: Changed 'ret' to 'ret_net'
# Extract exactly the 5bps net returns and join to perf_ts
...
p_sharpe <- ggplot(summary_drift, aes(x = threshold, y = sharpe)) +
  geom_line(color = WU_NAVY, linewidth = 1.2) +
  geom_point(size = 3, color = WU_BLUE) +
  geom_point(data = max_point, color = WU_GOLD, size = 5, shape = 18) +
  geom_label(
    aes(label = paste0(threshold, " bps")),
    vjust = -0.8,
    size = 3,
  ...
  theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold", color = WU_NAVY))
...
p_turnover <- summary_drift %>%
  ...
  scale_x_continuous(breaks = THRESHOLDS_BPS) +
  scale_color_manual(
    values = c("Avg monthly turnover" = WU_BLUE, "TC drag (% p.a.)" = WU_GOLD)
  ) +
  labs(
    x = "Drift‑band threshold (bps)",
    y = "Value",
  ...
  theme(
    legend.position = "bottom",
    plot.title = element_text(face = "bold", color = WU_NAVY)
  )
...
# =============================================================================
# 8. INDUSTRY EXPOSURE (FF17) STACKED AREA CHART & SUMMARY
# =============================================================================
cat("\n[6/6] Generating Industry Exposure Stacked Area Chart & Summary...\n")

# 1. Pull SIC codes from USA_FILE (fast read)
...
# 5. Define a High-Visibility 17-Color Palette
# This palette uses highly distinct, saturated colors to ensure separate industries don't blur together
high_vis_17 <- c(
  "#E6194B",
  "#3CB44B",
  "#FFE119",
...
# If 'Missing' creates an 18th category, colorRampPalette will safely expand it
...
# Format as percentages for clean printing
...
# Also warn if ANY industry ever breached 20%
...
p_tradeoff <- ggplot(
  ...
  scale_x_continuous(breaks = THRESHOLDS_BPS) +
  scale_color_manual(
    values = c("Gross_Sharpe" = "#1F78B4", "Net_Sharpe" = "#c8a951"), # Blue and Gold
...
# --- EXTRA PLOT: Cumulative Wealth (Gross vs Net Trade-off) ---
cat("\nGenerating Gross vs Net Cumulative Wealth Plot...\n")

# Define which thresholds to compare (Baseline vs a clear Drift Band)
compare_thresholds <- c(0, 5)

# 1. CRITICAL: Ensure data is sorted by date
setorder(drift_all, threshold, eom)
...
p_wealth_compare <- ggplot(
  ...
  scale_color_manual(
    values = c(
      "Gross (0 bps Drift)" = "#1F78B4", # Strong Blue (Highest line)
      "Net (0 bps Drift)" = "#A6CEE3", # Light Blue
      "Gross (5 bps Drift)" = "#c8a951", # Strong Gold (Collapses due to stale signal)
      "Net (5 bps Drift)" = "#E5D8BD" # Light Gold (Lowest line)
    )
  ) +
  scale_linetype_manual(
    values = c("Gross_Wealth" = "dashed", "Net_Wealth" = "solid"),
    guide = "none" # Hide the redundant linetype legend
  ) +
  labs(
    x = "Date",
    y = "Cumulative Wealth (log scale)",
  ...
  theme(
    legend.position = "right",
    plot.title = element_text(face = "bold", color = "#1a2e5a") # WU Navy
  )
...
# --- EXTRA PLOT: Cumulative Wealth (Gross vs Net with Shaded Fee Drag) ---
cat("\nGenerating Gross vs Net Cumulative Wealth Plot...\n")

# Define which thresholds to compare (Baseline vs 5 bps Drift Band)
compare_thresholds <- c(0, 5)
...
p_wealth_shaded <- ggplot(wealth_df, aes(x = eom, group = Strategy)) +
  ...
  # Custom Colors for the Shaded Areas
  scale_fill_manual(
